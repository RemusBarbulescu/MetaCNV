					...
					if (cnv[i].rdValue >= a_threshold && cnv[i].svValue <= a_threshold){
						if (!cnvNator.empty()){
							for (int k = 0; k < cnvNator.size(); ++k){
								...
								found = false;
								if (cnv[i].chr == cnvNator[k].chr && (cnv[i]._start >= cnvNator[k]._start && cnv[i]._start <= cnvNator[k]._end)){
								
									if (cnvNator[k].mutation == "deletion"){
										cnv_combined.emplace_back(cnv[i].chr, cnv[i]._start, cnv[i]._end, std::min(cnv[i].rdValue, cnv[i].svValue), cnv[i].rdValue, cnv[i].svValue, "Conflict 1 (RD:AMP, SV:DEL) + judge CNVnator: deletion");
										found = true;
										break;
									}
									else{
										cnv_combined.emplace_back(cnv[i].chr, cnv[i]._start, cnv[i]._end, std::max(cnv[i].rdValue, cnv[i].svValue), cnv[i].rdValue, cnv[i].svValue, "Conflict 1 (RD:AMP, SV:DEL) + judge CNVnator: amplification");
										found = true;
										break;
									}
								}
							...
							if (found == false){
								...
								cnv_combined.emplace_back(cnv[i].chr, cnv[i]._start, cnv[i]._end, 2.0, cnv[i].rdValue, cnv[i].svValue, "Conflict 1 (RD:AMP, SV:DEL, MetaCNV:ploidy )");
							...
							if (!cnvNator.empty()){
								for (int k = 0; k < cnvNator.size(); ++k){
									...
									cnv[i]._start <= cnvNator[k]._end)){
										...
										if (cnvNator[k].mutation == "deletion"){
											cnv_combined.emplace_back(cnv[i].chr, cnv[i]._start, cnv[i]._end, std::min(cnv[i].rdValue, cnv[i].svValue), cnv[i].rdValue, cnv[i].svValue, "Conflict 2 (RD:DEL, SV:AMP) + judge CNVnator: deletion");
											found = true;
											break;
										}
										else{
											cnv_combined.emplace_back(cnv[i].chr, cnv[i]._start, cnv[i]._end, std::max(cnv[i].rdValue, cnv[i].svValue), cnv[i].rdValue, cnv[i].svValue, "Conflict 2 (RD:DEL, SV:AMP) + judge CNVnator: amplification");
											found = true;
											break;
										}
									}
